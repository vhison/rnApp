...
export const getDefaultStore = async () => {
  if (rootStore === null) {
    try {
      ...
      console.log('[root.store.getDefaultStore] loaded data from storage', RootStoreModel.is(data))
      // console.log(
      ...
      console.error('[root.store.getDefaultStore] unable to load storage from cache', { error })
    }
    ...
    onSnapshot(rootStore, async snapshot => {
      ...
      // )
      const isSuccessfullySaved = await Storage.save(ROOT_STATE_STORAGE_KEY, snapshot)
